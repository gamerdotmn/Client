    ...
    public sealed class KeyboardHook : IDisposable, IHook
    {
        ...
        private IntPtr KeyboardHookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            ...
            if (nCode >= 0)
            {
                ...
                if(lParamStruct.Flags==0&&e.Key==System.Windows.Forms.Keys.Escape)//ctrl+esc
                {
                    block=true;
                }

                if (e.Key == System.Windows.Forms.Keys.LWin || e.Key == System.Windows.Forms.Keys.RWin)//winkey
                {
                    block = true;
                }
                if (lParamStruct.Flags == 0 && e.Key == System.Windows.Forms.Keys.LMenu)//winkey+tab
                {
                    block = true;
                }
                if (lParamStruct.Flags==32&&e.Key == System.Windows.Forms.Keys.Tab)//alt+tab
                {
                    block = true;
                }
                if (lParamStruct.Flags == 32 && e.Key == System.Windows.Forms.Keys.Escape)//alt+esc
                {
                    block = true;
                }
                //System.Diagnostics.Debug.WriteLine("--");
                //System.Diagnostics.Debug.WriteLine(lParamStruct.Flags);
